...
!      integer imn
!      real*8 rho_tmp(mr_n),rho_tmpL(mr_nL)
!      complex*16 workr2(mr_n)
!      character*20 filename,fileindex
...
ccccccccccccccccccccccccccc
      if(ibshift.gt.0) then
        call bandshift(kpt,iislda,E_st(1,kpt,iislda),AL,
     &                 totNel,iatom,xatom)
      endif
ccccccccccccccccccccccccccc
...
416   format(" err of ug            = ", 1(E11.4,1x))
417   format(" E_tot                = ",  E21.14,4x,E11.4)  
...
c       endif
     ...
     &            amx_mth(nint),0.8d0,0.25d0,0.1d0)  ! generate new vr_in_nL
c       if(inode==1) then
...
!
!       rho_tmp=0.d0
!       call d3fft_comp(ug_n_bp(1,imn),workr2,-1,1)
!       do i=1,mr_n
!        rho_tmp(i)=cdabs(workr2(i))**2
!       enddo
!       rho_tmpL=0.d0
!       call convert_SLvr(rho_tmp,rho_tmpL,1)
!
!       filename="rho."
!       write(fileindex,'(i6)') imn
!       filename=trim(adjustl(filename))//trim(adjustl(fileindex))
!       call rhoIO(AL,rho_tmpL,mr_nL,
!     &            1,n1L,n2L,n3L,filename)
!
!      enddo
...
666   continue
     ...
     &     (iislda-1)*nkpt+kpt.le.kpt_slda_dis(2)) then
      ...
      if(inode_tot.eq.1.and.nscf.gt.0) then
       ...
       write(22,415) Ef*27.211396d0 
415   format(" Ef(eV)               = ",  E14.7)
       write(22,401) dvE, dvE-dvE0
401   format(" dvE, dvE(n)-dvE(n-1) = ", 2(E11.4,1x))
       write(22,402) dv_ave,drho
402   format(" dv_ave, drho_tot     = ", 2(E11.4,1x))
       write(22,404) errave
404   format(" err of ug            = ", 1(E11.4,1x))
       write(22,*) "---------------------------------------------------"
       write(22,398) ewald
398   format(" Ewald        = ", E21.14)
       write(22,399) Ealphat
399   format(" Alpha        = ", E21.14)
       write(22,412) E_extV,E_extV-E_extV0
412   format(" E_extV       = ", E21.14,4x,E11.4)
       write(22,405) E_NSC, E_NSC-E_NSC0
405   format(" E_NSC        = ", E21.14,4x,E11.4)  
       write(22,406) E_cor, E_cor-E_cor0
406   format(" E[-rho*V_Hxc]= ", E21.14,4x,E11.4)  
       write(22,407) E_Hxc, E_Hxc-E_Hxc0
407   format(" E_Hxc        = ", E21.14,4x,E11.4)  
       write(22,408) -TS, -TS+TS0
408   format(" -TS          = ", E21.14,4x,E11.4)  
       write(22,409) E_tot, E_tot-E_tot0
409   format(" E_tot        = ", E21.14,4x,E11.4)  
       write(22,*) "---------------------------------------------------"
...
414    format("Zero temp. E_tot =", E21.14, 
     ...
     &  "ave(V_ion_s(or p,d))=ave(V_Hatree)=0; ave(Vtot)=ave(V_xc)=v0"
       write(22,*) "---------------------------------------------------"
396   format(" E_Hart,E_xc,E_ion =", 3(E17.10,2x))
397   format("    E_psiV,E_dDrho =", 2(E17.10,2x))
395   format("      ave(vtot):v0 =", E17.10)
413   format(" E_rhoVext,E_IVext     =", 2(E17.10,2x))
      call system_flush(22)
