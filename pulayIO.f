      subroutine pulayIO(AL,vr_n_tmp,mr_ni,npl,iflag,
...
c      if(icolor.eq.0) then    ! only read from the icolor group
     ...
     & status='old',action='read',iostat=ierr)
      ...
      read(15)n1t,n2t,n3t,nodes,nint_pulay_6,num_dwdR_6
...
ccccccccccccccccccccccccccccccc
     ...
     &                 =tempvr2
               ...
               if (isend==0) then
                  vr_n_tmp(1:nri/nnodes,ip)=tempvr1
               else
...
c     &        MPI_COMM_K,status,ierr)
         call mpi_recv(vr_n_tmp(1,ip),nri/nnodes,MPI_REAL8,0,100,
...
c      call mpi_bcast(vr_n_tmp,nri/nnodes,MPI_REAL8,0,MPI_COMM_N,ierr)
      call mpi_bcast(vr_n_tmp(1,ip),nri/nnodes,
     & MPI_REAL8,0,MPI_COMM_B2,ierr)
      call mpi_barrier(MPI_COMM_WORLD,ierr)
      call mpi_bcast(vr_n_tmp(1,ip),nri/nnodes,
     & MPI_REAL8,0,MPI_COMM_K2,ierr)
      enddo
...
c      if(icolor.eq.0) then
      ...
      if(icolor_b.eq.0.and.icolor_k.eq.0) then
       ...
       rewind(11)
       write(11) n1i,n2i,n3i,nnodes,nint_pulay_6,num_dwdR_6
       write(11) AL
       endif
       call mpi_barrier(MPI_COMM_K,ierr)
       ...
       if(inode.eq.1) then
       write(11) (vr_n_tmp(i,ip),i=1,n1i*n2i*n3i/nnodes)
       endif
       call mpi_barrier(MPI_COMM_K,ierr)
       ...
       if(inode==i+1) then
        do iloop=1,mr_ni
        temp_rho(iloop)=vr_n_tmp(iloop,ip)
        enddo
