...
c     calc current temperature, then make the output
      ...
      if(myid.eq.1) then
        ...
        write(11,*) "time,  TotalEn,  Etot,  Enki,  
     &               Enki_imp,  Temp,  Temp2, scaling, moment"
        write(11,666) 0,TotalEn,Etot*HFs2vB2M,Enki,Enki_imp,
     &   Temp,Temp,scaling,0,0,0,0,0,0
        close(11)
666     format(i5,1x,100(E18.9,1x))
      endif
...
c      implicit double precision (a-h,o-z)
      ...
      include 'param.escan_real'
******************************************
      real*8 xatom(3,matom),DeltR(3,matom)
      real*8 fatom0(3,matom),fatom1(3,matom)
      real*8 VX(matom),VY(matom),VZ(matom),V_output(3,matom)
      integer i,j,myid,istep,iscale,ntemp
      real*8 total_mass
      real*8 mv2,vcx0,vcy0,vcz0,vx0t,vy0t,vz0t,xx,yy,zz
      real*8 moment(3,2),vc(3,2),kinect(2),mass_tot(2),temp2
      ...
      do i = 1, ntemp
       ...
       mv2=mv2+MDatom(i)*(VX(i)**2+VY(i)**2+VZ(i)**2)
       !!!!!!!!!!!!!
       if(i.le.122.or.i.ge.151) then
         moment(1,1)=moment(1,1)+VX(i)*MDatom(i)
         moment(2,1)=moment(2,1)+VY(i)*MDatom(i)
         moment(3,1)=moment(3,1)+VZ(i)*MDatom(i)
         mass_tot(1)=mass_tot(1)+MDatom(i)
       else
         moment(1,2)=moment(1,2)+VX(i)*MDatom(i)
         moment(2,2)=moment(2,2)+VY(i)*MDatom(i)
         moment(3,2)=moment(3,2)+VZ(i)*MDatom(i)
         mass_tot(2)=mass_tot(2)+MDatom(i)
       endif
       !!!!!!!!!!!!!
      enddo

      do i=1,2
        vc(:,i) = moment(:,i) / mass_tot(i)
        kinect(i)=(moment(1,i)**2+moment(2,i)**2+moment(3,i)**2)/
     &            mass_tot(i)/2.d0
      enddo
      ...
      temp2= 2.d0*(Enki_old-kinect(1)-kinect(2))/
     &      (3.d0*DBLE(ntemp)*Hdt*HFs2vB2M)
      ...
      if(mod(istep,200).eq.0) then
        ...
        newtemp=dble(istep)/200.d0*30.d0
        if(temp2.gt.newtemp) newtemp=temp2

        do i=1,ntemp
         if(i.le.122.or.i.ge.151) then
          VX(i) = (VX(i) - vc(1,1)) * sqrt(newtemp/temp2) + vc(1,1)
          VY(i) = (VY(i) - vc(2,1)) * sqrt(newtemp/temp2) + vc(2,1)
          VZ(i) = (VZ(i) - vc(3,1)) * sqrt(newtemp/temp2) + vc(3,1)
         else
          VX(i) = (VX(i) - vc(1,2)) * sqrt(newtemp/temp2) + vc(1,2)
          VY(i) = (VY(i) - vc(2,2)) * sqrt(newtemp/temp2) + vc(2,2)
          VZ(i) = (VZ(i) - vc(3,2)) * sqrt(newtemp/temp2) + vc(3,2)
         endif
        enddo

        if(myid.eq.1) then
          write(6,*) "Artificially heating up"
          write(6,*) "New Temp=",newtemp
          write(6,*) "Scaling=",sqrt(newtemp/temp2)
          write(6,*)
          write(17,*) "Heating up scale=",sqrt(newtemp/temp2)
        endif
     ...
     &            mass_tot(i)/2.d0
      ...
      temp2= 2.d0*(Enki-kinect(1)-kinect(2))/
     &      (3.d0*DBLE(ntemp)*Hdt*HFs2vB2M)
     ...
     &  temperature,temp2,scaling,
     &  (moment(j,1),j=1,3),(moment(j,2),j=1,3)
        close(11)
666     format(i5,1x,100(E18.9,1x))
      endif
