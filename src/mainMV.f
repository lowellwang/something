...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!      count the calculation time, 11/30/2016 by ZHI
       real*8 :: t_start_petot
...
c     Calculate the approx. no. of g points, so we can dimension arrays
     ...
     &     +al(3,3)*(al(1,1)*al(2,2)-al(1,2)*al(2,1))
      ...
      mg_nx=int(1.5d0*totg/nnodes)+100

      if(inode_tot.eq.1) then
        write(6,*) "mg_nx = ",mg_nx
        write(22,*) "mg_nx = ",mg_nx
      endif
...
c       allocate(ug_all(mg_nx,nblock_band_mx,nkpt,islda))
...
c$$$!     end of sanity check
       ...
       call TDDFT(xatom,fatom,workr_n,Etot,
     ...
     &  amx_mth0,xgga,iwg_in,icoul,ivext_in,ilocal,iatom,ityatom,
     &  imov_at,totNel,Ealphat,t_start_petot)
       time11=mpi_wtime()
