...
******************************************
      ...
      if(mod(istep,200).eq.0) then
        ...
        if(temperature.gt.newtemp) newtemp=temperature
        VX(:)=VX(:)*sqrt(newtemp/temperature)
        VY(:)=VY(:)*sqrt(newtemp/temperature)
        VZ(:)=VZ(:)*sqrt(newtemp/temperature)

        if(myid.eq.1) then
          write(6,*) "Artificially heating up"
          write(6,*) "New Temp=",newtemp
          write(6,*) "Scaling=",sqrt(newtemp/temperature)
          write(6,*)
          write(17,*) "Heating up scale=",sqrt(newtemp/temperature)
        endif
